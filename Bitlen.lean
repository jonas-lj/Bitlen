...
bitLen n maxBits :=
  if maxBits ≤ 1 then
    n                                     -- n is 0 or 1, and is its own bit length
  else
    k := maxBits / 2
    if n >>> k = 0 then
      bitLen n k                          -- n fits in the low k bits; search there
    else
      k + bitLen (n >>> k) (maxBits - k)  -- the low k bits are significant; search above them
```
