...
/-- A right shift by `k` trades a bound of `2 ^ j` for a bound of `2 ^ (j + k)`. -/
...
/-- A right shift by `k` vanishes exactly when `n` fits in `k` bits. -/
...
/-- Any `n` of at least 2 bounded by `2 ^ maxBits` forces `maxBits` to be at least 2. -/
